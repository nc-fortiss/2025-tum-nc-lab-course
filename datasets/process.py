...
def show_repr(repr, fig=None, frame_rate=100,
        repeat=True, plot=False, save=False
    ):
    if fig is None:
        fig = plt.figure()
    ...
    image = plt.imshow(np.zeros((y_dim, x_dim, 3)), origin='lower')

    frames = np.zeros((repr.shape[0], y_dim, x_dim, 3))

    # precompute frames
    for i in range(repr.shape[0]):
        #frame = (repr[i] - repr[i].min()) / (repr[i].max() - repr[i].min())
        frame = repr[i] / (np.abs((repr[i])).max())
        frames[i, :, :, 0] = np.clip(frame, 0, 1)
        frames[i, :, :, 1] = -np.clip(frame, -1, 0)
