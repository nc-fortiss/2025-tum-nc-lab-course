...
class Assistant:
    ...
    def train(self, input, target):
        ...
        if self.count_log is True:
            if self.lam is None:
                ...
                output, rates, count = self.net(input)
        else:
            if self.lam is None:
                output = self.net(input)
            else:
                output, rates = self.net(input)

        loss = self.error(output, target)

        if self.extr_loss:
            net_loss = self.extr_loss(rates, target)
    ...
    def train_simclr(self, input, target):
        """Training assistant.
        ...
        output = self.net.forward_simclr(input)

        loss = self.error(output)
    ...
    def test_simclr(self, input, target):
        """Testing assistant.
    ...
    def train_combined(self, input, target):
        """Training assistant.
        ...
        output, feats = self.net(input)

        cls_error = self.error[0]
        cls_loss = cls_error(output, target)
        ssl_error = self.error[1]
        ssl_loss = ssl_error(feats)
    ...
    def test_combined(self, input, target):
        """Testing assistant.
